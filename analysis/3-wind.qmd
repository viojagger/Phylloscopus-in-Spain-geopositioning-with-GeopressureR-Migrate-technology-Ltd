...
-   These scripts should be adapted based on your project, but the same script should run for all your tags. You can always check [the original vesion online](https://github.com/Rafnuss/GeoPressureTemplate/blob/main/analysis/1-label.qmd).
...
```{r}
ecmwfr::wf_set_key("4076989b-33ef-476a-9abb-65d2e4357ac2")
```
...
tag <- geopressuretemplate_tag(id, quiet = F) #same as in script 4, it makes an RData file with pressure and light
```

## Download wind data: this will only download data to computer if there is nothing already in the directory. Otherwise, keep the old files
